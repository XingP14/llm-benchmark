...
> **v0.5.0 PR progress**: type declaration ✅ for all 14 entries (`webdev_arena` / `terminal_bench` / `aa_omniscience` / `benchlm_agentic` / `cyberseceval3` / **`swe_bench_pro`** / **`deepswe`** / **`long_context_cluster`** / `gpt_5_5_thinking_xhigh` / `gpt_5_4_thinking_xhigh` / `claude_opus_4_6_thinking` / `claude_mythos_5_1m` / `claude_opus_4_8_1m` / **`vllm_serving_bench`** / **`process_aware_scoring`**, 2026-06-10 00:24 + 06:43 + 2026-06-11 22:43 + 2026-06-12 02:43 + 2026-06-13 05:43 + 06-13 23:23 cron, this round extends 13→14 anchoring process-aware scoring) / `src/index.ts` console.info hint ✅ / `src/core/evaluator.ts` dispatch branches ✅ skeleton for all 8 entries (route entry, 2026-06-10 01:43 + 22:23 + 2026-06-12 03:23 + 2026-06-13 23:23 cron — this round extends `process_aware_scoring` to align with the full 14-entry type segment) / `src/web/engine/evaluator.ts` v0.5.0 dispatch hook point JSDoc annotation ✅ (2026-06-12 01:03 cron, hook point documented, real task-model extension pending 6 cron rounds) / `src/web/routes/evaluations.ts` config acceptance ⏳ — full PR estimated 30-45min (spanning 6-9 cron rounds); v0.5.0 will not be released before the full PR lands.
