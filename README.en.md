...
- Username: `admin`
- Password: from the `ADMIN_PASSWORD` environment variable; only development falls back to `admin123`, and production rejects the default password
...
The README now keeps only current user-facing entry points. v0.5.0 external benchmark candidates, GuideLLM companion-tool notes, and automation research history live in [`docs/EXTERNAL_BENCHMARK_RESEARCH.md`](docs/EXTERNAL_BENCHMARK_RESEARCH.md). These entries are planning notes until matching runtime code, tests, and config documentation land in source.
