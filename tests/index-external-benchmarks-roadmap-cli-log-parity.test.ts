...
// 钉住 src/index.ts L100-L108 cliLog 11/11 dispatch parity (Stirrup 11th fetcher closure).
// 6 regression gates 闭合 stale-comment silent-drift detection pattern:
// (1) cliLog message includes 'dispatch 11/11 真实化' (parity with DEFAULT_LOG_FORMAT 11-key map)
// (2) cliLog message lists 11 fetchers in fixed order, ending with Artificial Analysis Stirrup
// (3) JSDoc above cliLog reflects 'dispatch 11/11' (parity with message body)
// (4) DEFAULT_LOG_FORMAT in src/core/evaluator.ts declares 11 entries
// (5) DEFAULT_DISPATCH_TYPE in src/core/evaluator.ts declares 11 entries
...
describe('index external-benchmarks-roadmap cliLog 11/11 parity', () => {
  const indexSrc = fs.readFileSync(INDEX_PATH, 'utf-8');
  const evaluatorSrc = fs.readFileSync(EVALUATOR_PATH, 'utf-8');

  it('cliLog message includes "dispatch 11/11 真实化" (regression gate)', () => {
    const section = indexSrc.match(/v0\.5\.0\+ 外部基准路线图提示[\s\S]{0,1000}?\n    \}/);
    expect(section).not.toBeNull();
    expect(section![0]).toMatch(/dispatch 11\/11 真实化/);
  });

  it('cliLog message lists 11 fetchers in fixed order', () => {
    const section = indexSrc.match(/v0\.5\.0\+ 外部基准路线图提示[\s\S]{0,1000}?\n    \}/);
    expect(section).not.toBeNull();
    const expected11 = [
      'webdev_arena',
  ...
  it('JSDoc above cliLog reflects "dispatch 11/11" (parity with message body)', () => {
    const section = indexSrc.match(/v0\.5\.0\+ 外部基准路线图提示[\s\S]{0,1000}?\n    \}/);
    expect(section).not.toBeNull();
    const jsdoc = section![0].split('\n').find(l => l.includes('外部基准路线图提示'));
    expect(jsdoc).toBeDefined();
    expect(jsdoc).toMatch(/dispatch 11\/11 ✅/);
  });

  it('DEFAULT_LOG_FORMAT in src/core/evaluator.ts declares 11 entries', () => {
  ...
  it('DEFAULT_DISPATCH_TYPE in src/core/evaluator.ts declares 11 entries', () => {
  ...
  it('src/index.ts section has 0 hits of stale dispatch counts below 11/11', () => {
    const section = indexSrc.match(/v0\.5\.0\+ 外部基准路线图提示[\s\S]{0,1000}?\n    \}/);
    expect(section).not.toBeNull();
