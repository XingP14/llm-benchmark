...
//
// Pins:
// 1) dispatchV050External helper cfg type includes type?: ExternalDispatchType
// 2) dispatchV050External fetcher signature has 4th arg dispatchType: ExternalDispatchType
...
//    long_context_cluster) all declare `dispatchType: ExternalDispatchType = '<category>'` as last param
...
describe('evaluator v0.5.0 dispatch type wiring (5 fetcher payload dispatch_type)', () => {
  ...
  it('dispatchV050External cfg type includes type?: ExternalDispatchType field', () => {
    ...
    const sig = m![0];
    expect(sig).toMatch(/cfg:\s*\{[^}]*type\?:\s*ExternalDispatchType/);
  });

  it('dispatchV050External fetcher signature has 4th arg dispatchType: ExternalDispatchType', () => {
    const m = src.match(/private async dispatchV050External\([\s\S]*?\): Promise<void>/);
    expect(m).not.toBeNull();
    const sig = m![0];
    expect(sig).toMatch(/fetcher:\s*\(apiBase:\s*string,\s*model:\s*ModelConfig,\s*timeoutMs:\s*number,\s*dispatchType:\s*ExternalDispatchType\)/);
  });
  ...
  it.each(FIVE_NAMES)('$name fetcher declares dispatchType param via defaultDispatchType helper (chain #8 helper)', ({ shortFn }) => {
    // match: private async <shortFn>( ... dispatchType: ExternalDispatchType = defaultDispatchType('<name>') ) [chain #8 helper]
    const re = new RegExp(`private async ${shortFn}\\([\\s\\S]*?dispatchType:\\s*ExternalDispatchType\\s*=\\s*defaultDispatchType\\(['"]\\w+['"]\\)`);
