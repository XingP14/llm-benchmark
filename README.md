...
- 用户名：`admin`
- 密码：取自 `ADMIN_PASSWORD` 环境变量；仅开发环境未设置时回退到 `admin123`，生产环境会拒绝默认密码
...
README 只保留当前已实现的使用入口。v0.5.0 外部基准候选、GuideLLM 互补工具和自动化研究记录已移至 [`docs/EXTERNAL_BENCHMARK_RESEARCH.md`](docs/EXTERNAL_BENCHMARK_RESEARCH.md)。这些条目目前是规划材料；只有在对应运行时代码、测试和配置文档合入后，才视为已支持功能。
