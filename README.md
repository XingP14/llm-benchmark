...
**v0.5.0 PR 进度**: `src/types/index.ts` `ExternalBenchmarkRoadmap` + `BenchmarkConfig._external_benchmarks_roadmap?` 类型声明 ✅ (2026-06-10 00:24 cron) / `src/index.ts` console.info 启用提示 ✅ / `src/core/evaluator.ts` dispatch 分支 ⏳ / `src/web/routes/evaluations.ts` 配置验收 ⏳ — 真完整 PR 估 30-45min（跨 6-9 轮 cron 累进），v0.5.0 不发版。
