 ...
 */
/**
 * v0.6.0+ External benchmark dispatch category type union (07-07 cron, 5-dim type-stub closure step-N).
 *
 * 历史上 8 fetcher 接口各自硬编码单 literal type stub (type?: 'agentic_coding' 等),
 * 派生 client 调用时 TS narrowing 锁死在单一值, 无法表达「跨 5-dim dispatch 共享同一 union」的语义.
 * 现统一指向本 exported alias, 6 literal + `7th = agentic_coding × 2 callers (webdev_arena + terminal_bench) +
 * agentic_fullstack (benchlm_agentic) + agentic_swe (swe_bench_pro) + process_agentic (process_aware_scoring) +
 * long_context_retrieval (aa_omniscience + long_context_cluster) + safety_evaluation (cyberseceval3)`.
 *
 * 与 src/core/evaluator.ts DEFAULT_DISPATCH_TYPE (L51-L58, chain #8 helper-extraction) 对位 —
 * 6 literal default 全 alignment, 派生 client / web harness POST / reporter 5-dim 副标共享.
 *
 * 0 functional break: 所有 fetcher interface 的 `type?` 字段仍 optional, 缺省走 evaluator.ts ?? 'agentic_coding' fallback.
 * 唯类型层 closure: 8 个 stub 集中到 1 union alias, 防 6+1 literal 漂移.
 */
export type ExternalDispatchType =
  | 'agentic_coding'          // webdev_arena + terminal_bench (全栈 web dev / agentic terminal/shell)
  | 'agentic_fullstack'       // benchlm_agentic (Design2Code / Vision2Web / Native Evals)
  | 'agentic_swe'             // swe_bench_pro (multi-file / multi-lang patch eval)
  | 'process_agentic'         // process_aware_scoring (过程+结果双轨, 5 signals)
  | 'long_context_retrieval'  // aa_omniscience + long_context_cluster (知识检索 + 幻觉率)
  | 'safety_evaluation';      // cyberseceval3 (8 项 LLM 安全 / offensive security)
