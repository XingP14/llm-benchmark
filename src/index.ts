...
async function runBenchmark(args: string[]) {
  ...
  try {
    ...
    // v0.5.0+ 外部基准路线图提示 (roadmap-only, PR 进度: type ✅ 2150d07 / dispatch 11/11 ✅
    ...
    if (config._external_benchmarks_roadmap) {
      const enabled = Object.entries(config._external_benchmarks_roadmap)
        ...
        .map(([k]) => k);
      if (enabled.length > 0) {
        cliLog(`\n🧪 v0.5.0+ external benchmark roadmap detected: ${enabled.join(', ')} (PR progress: type ✅ 2150d07 / dispatch 11/11 真实化 (webdev_arena/terminal_bench/aa_omniscience/benchlm_agentic/cyberseceval3/swe_bench_pro/long_context_cluster/process_aware_scoring/lm_eval_task_conflict_resolver/livebench_2026_h1_quarterly_v3/artificial_analysis_stirrup_agent_framework_v1, per src/core/evaluator.ts line 161) / 真连接 adapter 仍 partial — full PR 估 30-45min)`);
      }
    }
